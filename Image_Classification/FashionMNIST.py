...
parser = argparse.ArgumentParser(description='FashionMNIST Example with Adam, SGDM, AdaSPID and FRSGDM')
...
parser.add_argument('--model', type=str, default='CNN', choices=['FNN', 'CNN'], help='model type')
...
class FNN(nn.Module):
    """Fully connected neural network for FashionMNIST classification"""
...
def get_optimizer(model, optimizer_name, learning_rate, momentum=0.9, gamma=0.01):
    ...
    if optimizer_name == 'Adam':
        return Adam(model.parameters(), lr=learning_rate, weight_decay=1e-3)
    elif optimizer_name == 'SGDM':
        return SGD(model.parameters(), lr=learning_rate, momentum=momentum, weight_decay=1e-3)
    elif optimizer_name == 'AdaSPID':
        return AdaSPID(model.parameters(), lr=learning_rate, momentum=momentum, gamma=gamma, weight_decay=1e-3)
    ...
    # Add RMSprop and AdamW
        ...
        return RMSprop(model.parameters(), lr=learning_rate, weight_decay=1e-3)
    elif optimizer_name == 'AdamW':
        return AdamW(model.parameters(), lr=learning_rate, weight_decay=1e-3)
    else:
...
if __name__ == '__main__':
    ...
    path = 'results/fashion_mnist'
    if not os.path.exists(path):
        os.makedirs(path)

    # Select model
    NN_set = {'FNN': FNN(), 'CNN': CNN()}
    initial_net = NN_set[args.model]

    # No longer need to save model structure to disk
